    ...
    public partial class BrigadeControl : UserControl
    {
        ...
        private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.RowIndex >= 0)
            {
                ...
                tbSurname.Text = row.Cells[1].Value.ToString();
                tbName.Text = row.Cells[2].Value.ToString();
                tbLastName.Text = row.Cells[3].Value.ToString();
                mtbPassNumber.Text = row.Cells[4].Value.ToString();
        ...
        private void UpdateTable()
        {
            ...
            for (int i = 0; i < dataGridView1.Rows.Count; i++)
            {
                ...
                if (rowState == RowState.Modified)
                {
                    ...
                    string PassNumber = dataGridView1.Rows[i].Cells[4].Value.ToString();
                    string Surname = dataGridView1.Rows[i].Cells[1].Value.ToString();
                    string Name = dataGridView1.Rows[i].Cells[2].Value.ToString();
                    string LastName = dataGridView1.Rows[i].Cells[3].Value.ToString();
                    string Phone = dataGridView1.Rows[i].Cells[5].Value.ToString();

                    string query = $"UPDATE BRIGADE set PassNumber = '{PassNumber}', Surname = '{Surname}', Firstname = '{Name}', Patronymic = '{LastName}', Phone = '{Phone}' WHERE Id = '{Id}'";
        ...
        private void SearchClient(DataGridView dgv)
        {
            ...
            string query = $"SELECT * FROM BRIGADE WHERE CONCAT (Id, PassNumber, Surname, Firstname, Patronymic, Phone) LIKE '%" + tbSearch.Text + "%'";
        ...
        private void Edit()
        {
            if (dataGridView1.CurrentCell != null)
            {
                ...
                dataGridView1.Rows[index].SetValues(id, Surname, Name, LastName, PassNumber, Phone);
